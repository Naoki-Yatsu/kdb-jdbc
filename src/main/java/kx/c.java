...
public class c {
    ...
    public static int n(Object x) throws UnsupportedEncodingException {
        return x instanceof Dict ? n(((Dict) x).x)
                : x instanceof Flip ? n(((Flip) x).y[0])
                        : x instanceof char[] ? new String((char[]) x).getBytes(e).length : Array.getLength(x);
    }
    ...
    /*
     * public static void main(String[]args){ // example tick feed boolean useTLS=false; c c=null; try{ c=new c("localhost",5010,System.getProperty("user.name"),useTLS); for(int i=0;i<10;i++){ //
     * Assumes a remote schema of mytable:([]time:`timespan$();sym:`symbol$();price:`float$();size:`long$() ) Object[]x={new Timespan(),"SYMBOL HERE",new Double(93.5),new Integer(300)};
     * c.ks(".u.upd","mytable",x); // send as async. .u.upd could just be insert } c.k(""); // sync chase ensures the remote has processed all msgs } catch(Exception e){e.printStackTrace(); }finally{
     * try{c.close();}catch(IOException e){} } }
     */
    /*
     * public static void main(String[]args){ // example tick subscriber boolean useTLS=false; c c=null; try{ c=new c("localhost",5010,System.getProperty("user.name"),useTLS); //
     * c.ks(".u.sub","mytable",x); while(true) O("Received "+c.k()); }catch(Exception e){e.printStackTrace(); }finally{ try{c.close();}catch(IOException e){} } }
     */
    /*
     * public static void main(String[]args){ // example client for query/response boolean useTLS=true; c c=null; try{ c=new c("localhost",5010,System.getProperty("user.name"),useTLS); Object
     * result=c.k("2+3"); O("result is "+result);
     * 
     * //Unicode example test //c.setEncoding("UTF-8"); //O("Unicode "+c.k("{`$x}","Ranby BjÃƒÂ¶rklund AB".toCharArray())); //O("Unicode "+c.k("{x}",(String)c.k("{`$x}",(char[])c.k(
     * "\"c\"$0x52616e627920426ac3b6726b6c756e64204142"))));
     * 
     * // select some data into a flip //Flip t=td(c.k("select sum size by sym from trade")); //O(n(t.x)); // number of columns //O(n(t.y[0])); // number of rows //O(at(t.y[0],0)); //data at 0,0
     * c.close(); }catch(Exception e){e.printStackTrace(); }finally{ try{c.close();}catch(IOException e){} } }
     */
    /*
     * public static void main(String[]args){ // example echo server for a single client boolean useTLS=false; int port=5010; c c=null; try{ ServerSocket s; if(useTLS){ SSLServerSocket
     * sslSocket=(SSLServerSocket)((SSLServerSocketFactory) SSLServerSocketFactory.getDefault()).createServerSocket(port); sslSocket.setEnabledCipherSuites(sslSocket.getSupportedCipherSuites());
     * s=sslSocket; } else s=new ServerSocket(port); c=new c(s); while(true) c.w(2,c.k()); } catch(Exception e){ e.printStackTrace(); } finally{ if(c!=null) try{c.close();}catch(IOException e){} } }
     */
}
